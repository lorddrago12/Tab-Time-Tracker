...
function activateTab(tabId) {
  ...
  document.querySelectorAll(".tab-panel").forEach((panel) => {
    panel.classList.toggle(
      "active",
      panel.id === document.getElementById(tabId).dataset.panel,
    );
  });
}
...
document.getElementById("modal-save").addEventListener("click", async () => {
  ...
  const redirect = normalizeUrl(
    document.getElementById("modal-redirect").value,
  );
  if (!site) return;
...
document.getElementById("add-btn").addEventListener("click", async () => {
  ...
  const exists = sites.some(
    (e) => (typeof e === "string" ? e : e.site) === site,
  );
  if (exists) {
...
async function applyCategoryToHistory(hostname, category) {
  await api.runtime.sendMessage({
    type: "reclassifyCategoryHistory",
    hostname,
    category,
  });
}
...
async function renderCategoryList() {
  ...
  entries.forEach(([site, category]) => {
    ...
    removeBtn.addEventListener("click", async () => {
      ...
      await applyCategoryToHistory(site, null);
      renderCategoryList();
...
document.getElementById("cat-add-btn").addEventListener("click", async () => {
  ...
  const site = normalizeHostname(siteInput.value);
